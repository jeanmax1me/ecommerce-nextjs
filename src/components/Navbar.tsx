    ...
    <div className="w-full h-20 border-b-[1px] border-b-zinc-500 bg-white text-zinc-600 sticky top-0 z-50 bg-white/80 backdrop-blur-2xl">
      <div className="max-w-screen-xl mx-auto h-full flex items-center justify-between px-4 xl:px-0">
          ...
          {navigation.map((item) => (
            <Link href={item?.href} key={item?._id}>
              ...
              >
                {item?.title}
                <span
                  className={`absolute h-[1px] w-full bg-designColor
        ...
        <div className="flex items-center gap-x-5">
          ...
          {session? (
        <Link href={"/profile"}  
        className="hover:text-black cursor-pointer duration-200 relative">
          Profile 
          <span className="absolute h-[1px] w-full bg-designColor left-0 bottom-0"></span>
        </Link>
          ) : (
            <button
            onClick={() => signIn()}
